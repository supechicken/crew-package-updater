...
class Vim < Package
  ...
  def self.check_update
    @_latest_ver = `git ls-remote --tags https://github.com/vim/vim`.scan(/refs\/tags\/v([^\^\n]*)/).flatten.sort_by do |ver|
      Gem::Version.new(ver)
    end[-1]

    return @_latest_ver, "v#{@_latest_ver}", git_tag: true
  end
end
